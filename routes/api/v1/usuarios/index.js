...
router.get('/byid/:codigo', async (req, res) => {
  try {
    ...
    const registro = await user.getUsuarioById({ codigo: (codigo) });
...
router.put('/update/:codigo', async (req, res) => {
  try {
    ...
    const {password , nombre, estado, avatar } = req.body;
    ...
    const updateResult = await user.updateUsuario({  email, password , nombre, estado, avatar ,codigo});
...
router.delete('/delete/:codigo', async (req, res) => {
  try {
    ...
    const deletedUsuario = await user.deleteUsuario({ codigo });
